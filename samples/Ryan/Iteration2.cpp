...
// tolerance is the number of decimal places desired.
    ...
    void Iteration2(Grid& A, int tolerance, int maxiterations)
    {
        int kkk=0;
        double derror=0;
        ...
        for(int i=0; i<A.nrows; i++)
        {
            for(int j=0; j<A.ncolumns; j++)
            {
            	...
            	else if(A.fixed[i][j]==0 && (i == 0) && (j != 0) && (j != A.ncolumns-1))
            	{
            		...
            		A.potential[0][j] = (1.0/3.0)*(A.potential[1][j]+A.potential[0][j+1]+A.potential[0][j-1]);
            	...
            	else if(A.fixed[i][j]==0 && (i==A.nrows-1) && (j!=A.ncolumns-1) && (j!=0))
            	{
            	    dold = A.potential[A.nrows-1][j];
         		    A.potential[A.nrows-1][j] = (1.0/3.0)*(A.potential[A.nrows-2][j]+A.potential[A.nrows-1][j+1]+A.potential[A.nrows-1][j-1]);
                ...
                else if(A.fixed[i][j]==0 && (j==0) && (i!=0) && (i!=A.nrows-1))
            	{
            		...
            		A.potential[i][0] = (1.0/3.0)*(A.potential[i][1]+A.potential[i+1][0]+A.potential[i-1][0]);
                ...
                else if(A.fixed[i][j]==0 && (j==A.ncolumns-1) && (i!=0) && (i!=A.nrows-1))
            	{
            	    dold = A.potential[i][A.ncolumns-1];
         		    A.potential[i][A.ncolumns-1] = (1.0/3.0)*(A.potential[i][A.ncolumns-2]+A.potential[i+1][A.ncolumns-1]+A.potential[i-1][A.ncolumns-1]);
            	...
            	{
            	    dold = A.potential[i][j];
            		A.potential[i][j] = 0.5*(A.potential[1][0]+A.potential[0][1]);
            		dnew = A.potential[i][j];
            		if (derror < (abs (dnew-dold)))
                    {
                        derror = abs (dnew-dold);
            	...
            	{
            	    dold = A.potential[i][j];
            		A.potential[i][j] = 0.5*(A.potential[A.nrows-2][j]+A.potential[A.nrows-1][j+1]);
            		dnew = A.potential[i][j];
            		if (derror < (abs (dnew-dold)))
                    {
                        derror = abs (dnew-dold);
            	...
            	{
            	    dold = A.potential[i][j];
            		A.potential[i][j] = 0.5*(A.potential[i][A.ncolumns-2]+A.potential[i+1][A.ncolumns-1]);
            		dnew = A.potential[i][j];
            		if (derror < (abs (dnew-dold)))
                    {
                        derror = abs (dnew-dold);
            	...
            	{
            	    dold = A.potential[i][j];
            		A.potential[i][j] = 0.5*(A.potential[A.nrows-2][j]+A.potential[i][A.ncolumns-2]);
            		dnew = A.potential[i][j];
            		if (derror < (abs (dnew-dold)))
                    {
                        derror = abs (dnew-dold);
